...
@router.get(
    "/projects/{project_id}/alerts", response_model=List[MonitoringAlertResponse]
)
...
async def update_alert_status(
    ...
    alert = (
        db.query(MonitoringAlert)
        .join(Project)
        .filter(MonitoringAlert.id == alert_id, Project.user_id == current_user.id)
        .first()
    )
    if not alert:
...
async def get_monitoring_settings_by_path(
    project_id: int,
    db: Session = Depends(deps.get_db),
    current_user=Depends(deps.get_current_user),
):
...
async def update_monitoring_settings_by_path(
    project_id: int,
