...
class NotificationService:
    ...
    async def send_email_notification(
        email: str, subject: str, template: str, data: Dict[str, Any]
    ) -> bool:
        """이메일 알림 전송"""
        try:
            # 템플릿에 데이터 적용
            ...
            # 이메일 전송
            await send_email_alert(email=email, subject=subject, message=message)
            return True
    ...
    @staticmethod
    async def send_webhook_notification(webhook_url: str, data: Dict[str, Any]) -> bool:
        """웹훅 알림 전송"""
        try:
